...
/**
 * 处理与日志记录相关的请求的Controller。
 * 继承自BaseController类并实现ILogRecordService接口。
 *
 * @version 1.0.0
 * @since 1.0.0
 * @author luoxiaodong
...
@RequestMapping("/api/infra/simple/crm/logRecord")
public class LogRecordController extends BaseController<LogRecord, ILogRecordService> {

    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(LogRecordController.class);

    @Autowired
    private ILogRecordService service;
    ...
    @Override
    public ILogRecordService getFeign() {
        return this.service;
    }
}
